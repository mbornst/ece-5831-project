...
class TrafficSigns:
    ...
    def _load_training(self):
        ...
        for i in range(42 + 1):
            ...
            for entry in obj:
                if entry.is_file() and (".ppm" in entry.name) and not(("cloudy.ppm" in entry.name) or ("sunny.ppm" in entry.name)) and not(("CW.ppm" in entry.name) or ("CCW.ppm" in entry.name)):
                    #print(entry.name)
...
# took 6:58 from run section to evaluate to complete

#%%
with open("traffic_signs_NN", 'wb') as f:
    pickle.dump(history, f, -1)
    
with open("traffic_signs_NN", 'rb') as f:
